...
export default function ImpactMatrix({
  ...
  const quadLabels = [
    { x: W - PAD - 6, y: PAD + 22,        anchor: 'end',   text: t('report.results.matrix.q1'), color: 'rgba(212,168,83,0.55)' },
    { x: PAD + 6,     y: PAD + 22,        anchor: 'start', text: t('report.results.matrix.q2'), color: 'rgba(96,165,250,0.55)' },
    { x: W - PAD - 6, y: H - PAD - 10,    anchor: 'end',   text: t('report.results.matrix.q3'), color: 'rgba(74,222,128,0.45)' },
    { x: PAD + 6,     y: H - PAD - 10,    anchor: 'start', text: t('report.results.matrix.q4'), color: 'rgba(140,140,140,0.4)' },
  ] as const;
  ...
  return (
    <div className="matrix-wrap">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="matrix-svg">
        ...
        <rect x={PAD + iW / 2}   y={PAD}            width={iW / 2} height={iH / 2} fill="rgba(212,168,83,0.05)" rx={3} />
        <rect x={PAD}            y={PAD + iH / 2}   width={iW / 2} height={iH / 2} fill="rgba(120,120,120,0.03)" rx={3} />
        <rect x={PAD + iW / 2}   y={PAD + iH / 2}   width={iW / 2} height={iH / 2} fill="rgba(74,222,128,0.03)" rx={3} />
        {/* Outer axis lines */}
        ...
        {/* Center cross (dashed) */}
        <line x1={PAD + iW / 2} y1={PAD}        x2={PAD + iW / 2} y2={H - PAD} stroke="rgba(255,255,255,0.06)" strokeWidth={1} strokeDasharray="4 5" />
        <line x1={PAD}          y1={PAD + iH / 2} x2={W - PAD}     y2={PAD + iH / 2} stroke="rgba(255,255,255,0.06)" strokeWidth={1} strokeDasharray="4 5" />
        {/* Axis arrows */}
        <polygon
        ...
        <text x={PAD + 8}     y={poleY} fontFamily="DM Mono,monospace" fontSize={10} fill="rgba(255,255,255,0.45)">− {axXlo}</text>
        <text x={W - PAD - 8} y={poleY} textAnchor="end" fontFamily="DM Mono,monospace" fontSize={10} fill="rgba(255,255,255,0.45)">{axXhi} +</text>
        <text x={22} y={PAD + 4} textAnchor="middle" fontFamily="DM Mono,monospace" fontSize={10} fill="rgba(255,255,255,0.45)" transform={`rotate(-90,22,${PAD + 4})`}>+ {axYhi}</text>
        <text x={22} y={H - PAD} textAnchor="middle" fontFamily="DM Mono,monospace" fontSize={10} fill="rgba(255,255,255,0.45)" transform={`rotate(-90,22,${H - PAD})`}>− {axYlo}</text>
        {/* Axis main labels (gold) */}
        <text x={W / 2} y={H - 14} textAnchor="middle" fontFamily="DM Mono,monospace" fontSize={11.5} fontWeight={500} letterSpacing="0.04em" fill="rgba(212,168,83,0.85)">{axX}</text>
        <text x={42} y={H / 2} textAnchor="middle" fontFamily="DM Mono,monospace" fontSize={11.5} fontWeight={500} letterSpacing="0.04em" fill="rgba(212,168,83,0.85)" transform={`rotate(-90,42,${H / 2})`}>{axY}</text>
        {/* Quadrant text labels */}
        {quadLabels.map((q, i) => (
          <text key={i} x={q.x} y={q.y} textAnchor={q.anchor} fontFamily="DM Mono,monospace" fontSize={10.5} fontWeight={500} letterSpacing="0.04em" fill={q.color}>
            {q.text}
          </text>
        ))}
        ...
        {nodes.map((n, i) => {
          ...
          return (
            ...
            >
              <circle cx={cx} cy={cy} r={n.r + 6} fill={n.color} opacity={0.10} />
              <circle cx={cx} cy={cy} r={n.r}     fill={n.color} opacity={0.88} />
              <text x={cx} y={cy + 4} textAnchor="middle" fontFamily="DM Mono,monospace" fontSize={11} fontWeight={600} fill="#09090b" pointerEvents="none">
                #{n.force.rank}
              </text>
            </g>
          );
