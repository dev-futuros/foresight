 ...
 * environments don't ship analytics to a real project. Set {@code POSTHOG_KEY} (and
 * optionally {@code POSTHOG_HOST}) in the environment to turn it on.
 *
 * @param enabled when {@code false}, {@link LlmCapture} becomes a no-op and the {@code PostHog}
 *                bean is not created. Auto-disabled when {@link #apiKey} is blank.
